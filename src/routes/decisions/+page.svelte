		...
		<tbody>
			<tr>
				...
				{#each tb.state_labels as state}
					<th class="bg-white">{state}</th>
				{/each}
			</tr>
			{#each tb.table as states, action_id}
				<tr>
					<th class="bg-white">{tb.action_labels[action_id]}</th>
					{#each states as payoff, state_id}
						{#if allow_input}
							<td class="bg-white"
								><input class="m-1 w-20" type="number" bind:value={states[state_id]} /></td
							>
						{:else}
							<td class="bg-white">{payoff}</td>
						{/if}
					{/each}
				</tr>
			{/each}
			...
			<tr>
				...
				{#each tb.state_probs as prob, prob_id}
					{#if allow_input}
						<td class="bg-white"
							><input
								class="m-1 w-20"
								...
								max="1"
								step="0.01"
								bind:value={tb.state_probs[prob_id]}
							/></td
						>
					{:else}
						<td class="bg-white">{prob.toFixed(3)}</td>
					{/if}
				{/each}
			</tr>
		</tbody>
	...
	<div class="flex min-h-0 flex-col gap-2">
		...
		<div class="flex flex-row flex-wrap items-baseline gap-2">
			<div class="light-border">
				<h2 class="text-center">Payoff table</h2>
				{@render format_table(tb, true)}
			</div>

			<div class="light-border">
				<h2 class="text-center">Risk table</h2>
		...
		<div class="flex flex-row flex-wrap gap-2">
			<div class="light-border">
				...
				<div>
					<b>Best action:</b>
					{max_min.best_actions.map((a) => tb.action_labels[a]).join(', ')}
				</div>
			...
			<div class="light-border">
				...
				<div>
					<b>Best action:</b>
					{max_max.best_actions.map((a) => tb.action_labels[a]).join(', ')}
				</div>
			...
			<div class="light-border">
				...
				<div>
					<b>Best action:</b>
					{alt_max_min.best_actions.map((a) => tb.action_labels[a]).join(', ')}
				</div>
				<div>
					<b>in state:</b>
					{alt_max_min.best_states.map((a) => tb.state_labels[a]).join(', ')}
				</div>
				<div><b>Risk:</b> {alt_max_min.best_payoff}</div>
			</div>
			...
			<div class="light-border">
				...
				<div>
					<b>Best action:</b>
					{laplace_choice.best_actions.map((a) => tb.action_labels[a]).join(', ')}
				</div>
			...
			<div class="light-border">
				...
				<div class="flex flex-row gap-2">
					<div class="w-8">{alpha}</div>
					<input class="grow" bind:value={alpha} type="range" min="0" max="1" step="0.001" />
				</div>
				<hr />
				<div>
					<b>Best action:</b>
					{realism_choice.best_actions.map((a) => tb.action_labels[a]).join(', ')}
				</div>
			...
			<div class="light-border">
				...
				<div>
					<b>Best action:</b>
					{emv_choice.best_actions.map((a) => tb.action_labels[a]).join(', ')}
				</div>
