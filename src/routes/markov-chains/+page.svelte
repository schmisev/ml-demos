<script lang="ts">
	...
	let { model: hmm, evidence_templates } = $state(chosen_model());
	...
	function update_graphs() {
		...
		graph = hmm.format_graph_for_hui('filter', show_evidence, colormap);
	}
...
<div class="flex flex-col gap-2 p-2">
	<div class="flex flex-row items-center gap-5">
		...
		<label class="light-border">
			...
			<select bind:value={chosen_model} onchange={reset}>
				...
				<option value={RAIN_TEMP_UMBRELLA_TSHIRT_HMM}
					>Rain, Temperature, Umbrellas & T-Shirts</option
				>
			</select>
		</label>
	</div>
	<div class="flex flex-row">
		<div class="flex grow flex-col">
			...
			string={graph_str}
		></Mermaid-->
			<div class="flex max-h-100 flex-col items-center">
			...
			<div class="flex flex-row flex-wrap gap-2">
				...
				<button class="positive border" onclick={filter}>Filter ▷</button>
				<button class="special border" onclick={backward}>Backward ◁</button>
			</div>
		</div>
		...
		<div class="light-border flex flex-col flex-wrap gap-2">
			...
			<label class="light-border"
				>Show evidence in graph: <input
					type="checkbox"
					bind:checked={show_evidence}
					onchange={update_graphs}
				/></label
			>
		</div>
	</div>
