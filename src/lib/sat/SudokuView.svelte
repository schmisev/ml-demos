...
<div class="light-border">
  <div class="grid aspect-square grid-cols-4 grid-rows-4">
    {#each grid as row, i}
      {#each row as cell, j}
        <div style="{cell !== undefined ? `background-color: ${colormap[cell]}` : ""}" class="flex flex-row items-center justify-center text-center border-1 {(j === 1 ? "border-r-2" : "")} {(i === 1 ? "border-b-2" : "")}">{cell}</div>
      {/each}
    {/each}
  </div>
</div>
